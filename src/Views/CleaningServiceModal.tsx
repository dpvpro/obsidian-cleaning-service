import {
	CleaningServiceView,
	CleaningServiceViewProps,
	SelectableItem,
} from "./CleaningServiceView";
...
export class CleaningServiceModal extends Modal {
	...
	applySelectionChangeToAllSections(names: string[], value: boolean) {
		this.state = {
			...
			orphans:
				this.state.orphans &&
				changeSelection(this.state.orphans, names, value),
			empty:
				this.state.empty &&
				changeSelection(this.state.empty, names, value),
			big:
				this.state.big && changeSelection(this.state.big, names, value),
			expired:
				this.state.expired &&
				changeSelection(this.state.expired, names, value),
			emptyDirectories:
				this.state.emptyDirectories &&
				changeSelection(this.state.emptyDirectories, names, value),
		};
	}
	...
	public updateState(results: ScanResults) {
		this.state = {
			...
			emptyDirectories: this.folderToSelectableItem(
	...
	private folderToSelectableItem(
	...
	render() {
		this.root.render(
			<React.StrictMode>
				<CleaningServiceView {...this.state} />
			</React.StrictMode>,
		);
	}
