...
export class FileScanner {
	...
	async scan() {
		...
		const vaultConfig = (
			this.app.vault as { config?: { userIgnoreFilters?: string[] } }
		).config;
		if (
			this.settings.honorObsidianExcludedFiles &&
			vaultConfig?.userIgnoreFilters
		) {
			exclusionFilters = exclusionFilters.concat(
				vaultConfig.userIgnoreFilters,
			);
		}

		const regexes = exclusionFilters.map<RegExp>(
			(filter: string) => new RegExp(filter, "i"),
		);
	...
	private findBigFiles(files: TFile[]) {
		return files.filter(
			(file) => file.stat.size >> 10 > this.settings.sizeLimitKb,
		);
	}

	private findEmptyDirectories(): string[] {
		const allDirectories = this.app.vault
			.getAllLoadedFiles()
		...
		const expired = frontMatters
			.filter((fm) => {
				const expires = fm.frontMatter[
					this.settings.expiredAttribute
				] as string | undefined;
				if (expires) {
					//https://day.js.org/docs/en/parse/string-format
					const maybeDate = moment(
						expires,
						this.settings.expiredDateFormat,
					);
	...
	private async findOrphans(
		...
		const resolvedLinks: { [key: string]: number } =
	...
	async getCanvasResources(canvases: TFile[]) {
		...
		const resources = datas.reduce(
			(acc: { [key: string]: number }, data: CanvasData) => {
						...
						case "text":
							...
							while ((m = regex.exec(textNode.text)) !== null) {
								...
								if (res) {
									...
									const attPath = normalizePath(
										`${vaultConfig?.attachmentFolderPath || ""}/${res}`,
									);
	...
	private getOrphans(
		others: TFile[],
		resolvedLinksAndResolvedProps: { [key: string]: number },
	) {
	...
	private combineLinksAndResolvedMetadata(
		frontMatters: (
			| {
					frontMatter: FrontMatterCache;
					stringProps: string[];
		...
		resolvedLinks: { [key: string]: number },
	) {
		return frontMatters.reduce((rl, fm) => {
			if (fm?.resolvedProps?.length) {
				return {
					...rl,
					...fm.resolvedProps.reduce(
						(ob, val) => ({ ...ob, [val]: 1 }),
						{},
					),
				};
			...
			.map((file) => {
				...
				if (frontMatter) {
					const stringProps: string[] =
						extractStringProperties(frontMatter);
					if (stringProps?.length) {
						// we should distinguish files from other props maybe...
						const resolvedProps: string[] = stringProps
							.map((sp) => {
								const resolvedFile =
	...
	private getResolvedLinks() {
		const resolvedLinks: { [key: string]: number } = Object.keys(
...
function extractStringProperties(fm: Record<string, unknown>): string[] {
	return Object.values(fm).filter((o): o is string => typeof o === "string");
}
